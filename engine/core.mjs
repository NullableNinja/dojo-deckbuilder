...
export class Game {
  constructor(data,{seed=1,strategies=["balanced","balanced"],characters=[]}={}){
    this.data=data; this.rng=new Rng(seed); this.round=1; this.turns=0; this.winner=null; this.reason=""; this.events=[]; this.cardStats=new Map(); this.marketPurchasedThisRound=false;
  ...
  resolveAttack(attacker,defender,card,{useTempo=true,defenseCard}={}){
    ...
    if(defense){remove(defender.hand,defense);defender.discard.push(defense);defender.focus+=focus(defense);defenseFocus=focus(defense);this.track(defense,"played",defender);}
  ...
  run(){while(!this.checkWinner()){for(let i=0;i<2&&this.winner===null;i++)this.botTurn(i);if(this.winner===null){this.round++;for(const p of this.players){p.xp+=this.data.definition.progression.xpPerHonor;p.tempo=true;}if(!this.marketPurchasedThisRound)this.refillMarket(true);this.marketPurchasedThisRound=false;}}for(const p of this.players)for(const id of p._used??[])if(p.id===this.winner)this.cardStats.get(id).winnerOwned++;return this.result();}
