...
export class Game {
  ...
  resolveAttack(attacker,defender,card,{useTempo=true,defenseCard}={}){
    ...
    if(defense){remove(defender.hand,defense);defender.played.push(defense);defense.focus+=focus(defense);defenseFocus=focus(defense);this.track(defense,"played",defender);}
  ...
  botTurn(index){const p=this.players[index],d=this.players[1-index];p.tempo=true;this.practice(p,choosePractice(p.hand,p.strategy));for(let n=0;n<this.data.definition.turn.normalAttackLimit;n++){const a=chooseAttack(p.hand,p.strategy);if(!a)break;this.resolveAttack(p,d,a,{useTempo:n===0});if(d.hp<=0)break;}for(const card of [...p.hand]){if(["Kata","Consumable","Gear","Weapon","Defense Equipment"].includes(card.subtype)){remove(p.hand,card);p.played.push(card);p.focus+=focus(card);this.track(card,"played",p);}}
    while(true){const buy=choosePurchase(this.market,p.focus,p.strategy);if(!buy||!this.buy(p,buy))break;}this.hide(p);this.turns++;this.checkWinner();
  }
  ...
  result(){return {winner:this.winner,reason:this.reason,rounds:this.round,turns:this.turns,players:this.players.map(p=>({id:p.id,strategy:p.strategy,hp:p.hp,purchases:p.purchases,plays:p.plays,openingPurchase:p.openingPurchase})),cards:[...this.cardStats.values()],events:this.events};}
}
