...
const CARD_KEYS = ["offered", "drawn", "purchased", "played", "discarded", "destroyed", "hits", "blocks", "damageDealt", "damagePrevented", "effectApplications", "equipmentReadied", "equipmentExhausted"];
...
function makeSummary({ args, data, mode }) {
  return {
    ...
    mode: { id: mode.id, definition: mode }, policy: args.policy, gamesRequested: args.games, seedStart: args.seedStart, workers: args.workers, telemetryDetail: args.detail,
    ...
    economy: { openingPurchasePlayers: 0, openingPurchaseRate: 0, totalPurchases: 0, averagePurchasesPerPlayer: 0 }, progression: { averageXpPerPlayer: 0, maxXp: 0, promotions: 0, beltDistribution: {} },
    strategies: {}, matchups: {}, cards: {}, families: {}, failures: [], longestGames: [], replaySeeds: [], output: { games: args.gameOutput },
  };
}

function addGame(summary, result, roundValues, turnValues) {
  if (!result.ok) {
  ...
  for (const player of result.players) { const strategy = summary.strategies[player.strategy] ??= { games: 0, wins: 0, winRate: 0 }; strategy.games += 1; if (player.id === result.winner) strategy.wins += 1; summary.economy.totalPurchases += player.purchases; summary.economy.openingPurchasePlayers += player.openingPurchase ? 1 : 0; const belt = String(player.beltIndex); summary.progression.beltDistribution[belt] = (summary.progression.beltDistribution[belt] ?? 0) + 1; summary.progression.maxXp = Math.max(summary.progression.maxXp, player.xp); summary.progression.averageXpPerPlayer += player.xp; }
...
async function writeReports(summary, output) {
  await mkdir(dirname(output), { recursive: true }); await writeFile(output, `${JSON.stringify(summary, null, 2)}\n`);
    ...
    worker.on("message", (message) => {
      try {
        if (message.type === "game") {
          addGame(summary, message.result, roundValues, turnValues);
