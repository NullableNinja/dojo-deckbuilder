Warning: truncated output (original token count: 81122)
Total output lines: 4061
...
function ImpactReadout({ exchange, line }: { exchange?: PlaytestCombatExchange | null; line: string }) {
  if (exchange) {
    ...
    return <blockquote className={`impact-readout exchange-receipt is-${exchange.outcome}`} key={exchange.id}>
      <header><span>{exchange.outcome === "hit" ? "Impact certified" : "Block certified"}</span><b>{exchange.zone} · {exchange.isReversal ? "REVERSAL…21122 tokens truncated…") return current;
