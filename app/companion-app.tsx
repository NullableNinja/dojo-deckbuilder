...
const ReferenceDesk = lazy(() => import("./reference-desk").then((module) => ({ default: module.ReferenceDesk })));
...
const COMPLETE_CARD_URLS_BY_CATALOG_ID = Object.fromEntries(
  Object.entries({ ...ATTACK_CARD_URLS, ...DEFENSE_CARD_URLS, ...KATA_CARD_URLS, ...CONSUMABLE_CARD_URLS, ...DEFENSE_EQUIPMENT_CARD_URLS, ...GEAR_CARD_URLS, ...COMBO_CARD_URLS, ...STARTER_CARD_URLS }).flatMap(([path, url]) => {
...
const MOBILE_MENU_ITEMS: { id: ViewId; label: string; detail: string }[] = [
  ...
  { id: "reference", label: "Reference Desk", detail: "Canonical table lookup from generated game data." },
...
export default function CompanionApp() {
  ...
  return <div className="site-frame">
    ...
    <div key={view} className="view-stage">{view === "home" && <HomeView goTo={goTo} />}{view === "playtest" && <Suspense fallback={null}><PlaytestView goTo={goTo} /></Suspense>}{view === "reference" && <Suspense fallback={null}><ReferenceDesk goTo={goTo} /></Suspense>}{view === "quickstart" && <QuickStartView goTo={goTo} />}{view === "story" && <StoryView goTo={goTo} />}{view === "rules" && <RulesView key={`${searchedRuleChapter || "rules"}-${searchedRuleSection}`} initialChapterId={searchedRuleChapter} initialSectionId={searchedRuleSection} />}{view === "cards" && <CardsView initialCard={searchedCard} clearInitialCard={() => setSearchedCard(null)} />}{view === "rulings" && <RulingsView key={searchedRuling || "rulings"} initialQuery={searchedRuling} />}{view === "glossary" && <GlossaryView key={searchedTerm || "glossary"} initialQuery={searchedTerm} />}{view === "house-rules" && <HouseRulesView key={searchedHouseRule || "house-rules"} initialQuery={searchedHouseRule} />}</div>
