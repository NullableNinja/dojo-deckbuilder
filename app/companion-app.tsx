...
const PHASES = [
  ...
  { letter: "A", name: "Ascend", text: "Spend Focus, buy cards, control refills, and promote." },
...
const GAME_MODES = [
  ...
  { id: "quick-duel", label: "Fast", title: "Quick Duel: Face-Punch Finals", players: "Exactly 2 players", fighters: "1 Character each", win: "Last Fighter Standing only", detail: "A fast 1v1 combat-testing format. Belt progression still operates, but promotions never increase Max HP or heal. Every other Belt perk remains active, and reaching Black Belt does not end the game.", notes: ["Both fighters remain at 25 Max HP unless a card or scenario says otherwise.", "Black Belt Victory is disabled unless a scenario restores it.", "Use normal initiative, Tempo, combat, Defense Practice, and Controlled Refill rules."] },
...
export default function CompanionApp() {
  ...
  const globalResults: GlobalResult[] = useMemo(() => {
    ...
    const rulings: GlobalResult[] = OFFICIAL_RULINGS.filter((entry) => Object.values(entry).join(" ").toLocaleLowerCase().includes(term)).slice(0, 2).map(({ id, filed, tag, title }) => ({ type: "Ruling", title, detail: `${id} · ${tag} · ${filed}`, view: "rulings", query: title }));
    ...
    const terms: GlobalResult[] = GLOSSARY_ENTRIES.filter((entry) => `${entry.term} ${entry.meaning}`.toLocaleLowerCase().includes(term)).slice(0, 2).map((entry) => ({ type: "Glossary", title: entry.term, detail: entry.meaning, view: "glossary", query: entry.term }));
