...
export type EquipmentHitResolution = {
  ...
  grantFlow: boolean;
  directDamage: number;
  delayedStatuses: Array<{ sourceEffectId: string; effect: "combat.modifySpeed" | "combat.modifyDefense" | "combat.dealDamage" | "combat.modifyHealing"; amount: number; duration: "nextRound" | "nextInitiate" | "endOfTargetNextTurn"; target: "self" | "opponent" }>;
...
export function structuredEquipmentHitResolution(cards: EquipmentCardLike[], context: EquipmentHitContext): EquipmentHitResolution {
  ...
  for (const card of cards) {
    ...
    for (const effect of hitEffects) {
      ...
      else if (effect.effect === "core.custom" && effect.target === "opponent" && equipmentConditionValue(effect, "scheduledTiming") === "beforeControllerNextTurn") result.delayedStatuses.push({ sourceEffectId: effectId, effect: "combat.modifyHealing", amount, duration: "nextInitiate", target: "opponent" });
