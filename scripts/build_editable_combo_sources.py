#!/usr/bin/env python3
"""Render Paper-Fu Learned Combo card faces and GIMP-compatible ORA sources.
...
def tab_layer(kind: str) -> Layer:
    ...
    label = "LEARNED · COMBO" if kind == "combo" else ("ITEM · GEAR" if kind == "gear" else "ITEM · CONSUMABLE")
...
def stat_layer(card: dict, kind: str) -> Layer:
    ...
    # Learned Combos are not equipment.  Their printed information is Focus
    # cost, a deliberately blank Power/Guard value, and their learned status.
    # Keeping those fields distinct prevents the Gear template's "Item" slot
    # from leaking onto Combo cards.
    guard = "—" if kind == "combo" else str(stats.get("Guard", stats.get("Use", stats.get("Uses", "—"))))
    slot = "LEARNED" if kind == "combo" else str(stats.get("Slot", "Item"))
    ...
    return Layer("07 · Focus, Combo value, and learned status", image)
...
def normalize_combo_art_sources() -> None:
    """Make compact alpha-clean source assets for the published card build."""
    source_dir = ROOT / "app/assets/art/combos/raw"
    ready_dir = ROOT / "app/assets/art/combos/ready"
    ready_dir.mkdir(parents=True, exist_ok=True)
    for source in source_dir.glob("ddb-cmb-core-*.png"):
        strip_external_matte(Image.open(source)).save(ready_dir / source.name, optimize=True)


def combo_art_from_source(card: dict) -> Layer:
    """Place one authored Paper-Fu illustration on its own editable card layer."""
    card_id = card["catalogId"].lower()
    candidates = list((ROOT / "app/assets/art/combos/ready").glob(f"{card_id}_*.png"))
    if not candidates:
...
def card_layers(card: dict, kind: str) -> list[Layer]:
    ...
    layers.append(combo_art_from_source(card) if kind == "combo" else (defense_art_from_source(card) if kind == "gear" else source_art_from_flat(card)))
    title_canvas = rgba()
...
def build_combo_sources(output: Path) -> None:
    source_dir = output / "combos"
    web_dir = ROOT / "app/assets/cards/combos"
    web_dir.mkdir(parents=True, exist_ok=True)
    cards = [card for card in CARDS if card.get("catalogId", "").startswith("DDB-CMB-CORE-")]
    if len(cards) != 55:
        raise RuntimeError(f"Expected 55 Learned Combo cards, found {len(cards)}")
    for card in cards:
        base = f"{card['catalogId'].lower()}_{slug(card['name'])}"
        layers = card_layers(card, "combo")
        write_ora(source_dir / f"{base}.ora", card, layers)
        merged(layers).convert("RGB").save(web_dir / f"{base}.webp", "WEBP", quality=88, method=6)
    archive_ora(source_dir, ROOT / "public/downloads/Dojo_Deckbuilder_v2.3_Learned_Combos_Editable_ORA.zip", "Dojo Deckbuilder v2.3 — Learned Combo Editable Sources")
...
def main() -> int:
    ...
    build_combo_sources(output)
    print("Created 55 Learned Combo card faces and 55 editable ORA source files.")
    return 0


if __name__ == "__main__":
    if "--normalize-art" in sys.argv:
        normalize_combo_art_sources()
    else:
