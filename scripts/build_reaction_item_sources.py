#!/usr/bin/env python3
"""Render Paper-Fu Reaction Item card faces and GIMP-compatible ORA sources.
...
def normalize_defense_art_sources() -> None:
    ...
    source_dir = ROOT / "app/assets/art/gear/raw"
    ready_dir = ROOT / "app/assets/art/gear/ready"
    ready_dir.mkdir(parents=True, exist_ok=True)
    for source in source_dir.glob("ddb-gea-core-*.png"):
        strip_external_matte(Image.open(source)).save(ready_dir / source.name, optimize=True)


def reaction_art_from_source(card: dict) -> Layer:
    """Place one authored Paper-Fu illustration on its own editable card layer."""
    card_id = card["catalogId"].lower()
    candidates = list((ROOT / "app/assets/art/reactions/source").glob(f"{card_id}_*.png"))
    if not candidates:
        raise FileNotFoundError(f"Missing authored Paper-Fu Reaction illustration for {card['catalogId']}")
    raw = Image.open(candidates[0]).convert("RGBA")
    raw.thumbnail((410, 270), Image.Resampling.LANCZOS)
    width, height = raw.size
    image = rgba()
    image.alpha_composite(raw, ((WIDTH - width) // 2, 183 + (278 - height) // 2))
    return Layer("04 · Authored Paper-Fu Reaction illustration", image)


def card_layers(card: dict, kind: str) -> list[Layer]:
    layers = template_layers(card, kind)
    layers.append(tab_layer(kind))
    layers.append(reaction_art_from_source(card))
    title_canvas = rgba()
...
def build_reaction_sources(output: Path) -> None:
    source_dir = output / "reactions"
    web_dir = ROOT / "app/assets/cards/reactions"
    web_dir.mkdir(parents=True, exist_ok=True)
    cards = [card for card in CARDS if card.get("catalogId", "").startswith("DDB-RIT-CORE-")]
    if len(cards) != 12:
        raise RuntimeError(f"Expected 12 Reaction Item cards, found {len(cards)}")
    requested = {value.strip().upper() for value in os.environ.get("DDB_CARD_IDS", "").split(",") if value.strip()}
    if requested:
        cards = [card for card in cards if card["catalogId"].upper() in requested]
        found = {card["catalogId"].upper() for card in cards}
        if found != requested:
            raise RuntimeError(f"Unknown Reaction Item IDs: {', '.join(sorted(requested - found))}")
    for card in cards:
        base = f"{card['catalogId'].lower()}_{slug(card['name'])}"
        layers = card_layers(card, "reaction")
        write_ora(source_dir / f"{base}.ora", card, layers)
        merged(layers).convert("RGB").save(web_dir / f"{base}.webp", "WEBP", quality=88, method=6)
    batch_name = os.environ.get("DDB_BATCH_NAME", "Complete")
    archive_ora(source_dir, ROOT / "public/downloads" / f"Dojo_Deckbuilder_Reaction_Items_{batch_name}_Editable_ORA.zip", "Dojo Deckbuilder — Reaction Item Editable Sources")
    print(f"Created {len(cards)} Reaction Item card faces and {len(cards)} editable ORA source files.")
