#!/usr/bin/env python3
"""Render Paper-Fu Gear card faces and GIMP-compatible ORA sources.
...
def tab_layer(kind: str) -> Layer:
    ...
    label = "ITEM · GEAR" if kind == "gear" else "ITEM · CONSUMABLE"
...
def defense_art_from_source(card: dict) -> Layer:
    ...
    card_id = card["catalogId"].lower()
    candidates = list((ROOT / "app/assets/art/gear/ready").glob(f"{card_id}_*.png"))
    if not candidates:
...
def card_layers(card: dict, kind: str) -> list[Layer]:
    ...
    layers.append(defense_art_from_source(card) if kind == "gear" else source_art_from_flat(card))
    title_canvas = rgba()
...
def build_gear_sources(output: Path) -> None:
    source_dir = output / "gear"
    web_dir = ROOT / "app/assets/cards/gear"
    web_dir.mkdir(parents=True, exist_ok=True)
    cards = [card for card in CARDS if card.get("catalogId", "").startswith("DDB-GEA-CORE-")]
    if len(cards) != 24:
        raise RuntimeError(f"Expected 24 Gear cards, found {len(cards)}")
    for card in cards:
        base = f"{card['catalogId'].lower()}_{slug(card['name'])}"
        layers = card_layers(card, "gear")
        write_ora(source_dir / f"{base}.ora", card, layers)
        merged(layers).convert("RGB").save(web_dir / f"{base}.webp", "WEBP", quality=88, method=6)
    archive_ora(source_dir, ROOT / "public/downloads/Dojo_Deckbuilder_v2.3_Gear_Editable_ORA.zip", "Dojo Deckbuilder v2.3 — Gear Editable Sources")
...
def main() -> int:
    ...
    build_gear_sources(output)
    print("Created 24 Gear card faces and 24 editable ORA source files.")
    return 0
