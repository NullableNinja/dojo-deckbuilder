...
presentation = presentation.replace(
  /const beltRewardsChangeHp = gameDefinition\.progression\.belts\.some\(\(belt\) => \{[\s\S]*?\n\};\n\nconst hydrateMechanicalText = \(text: string\) => \{[\s\S]*?\n\};\n\nconst hydrateBlock = \(block: RuleBlock\): RuleBlock => \{[\s\S]*?\n\};\n\nexport const CANONICAL_RULES = \{[\s\S]*?\n\} satisfies RuleData;/,
  "export const CANONICAL_RULES = rawRules satisfies RuleData;",
);
...
test = test.replace(
  /test\("player-facing rules presentation is driven by canonical JSON", async \(\) => \{[\s\S]*?\n\}\);/,
